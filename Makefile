...
#   make CFLAGS=-O2
#   make install DESTDIR=$(CURDIR)/debian/modmeter.lv2 PREFIX=/usr
#
...
modmeter_VERSION?=$(shell git describe --tags HEAD 2>/dev/null | sed 's/-g.*$$//;s/^v//' || echo "LV2")
###############################################################################
LIB_EXT=.so
